...
[t,y] = ode45(@(t,y) flightpath_ode(t,y), [0 500], init_cond);
% Units convert:
...
function [dydt] = flightpath_ode(t,y)
    ...
    if round(Z,-3) ~= 0
        ...
        area = 0.20828*0.09652; % Estimated size of largest face (review)

        %% HEY CADE CHECK THIS DRAG EQUATION AND CHANGE IT FROM HERE DOWN
        Drag = 0.5*cd*vZ.^2.*rho*area; 
    
        F_net =  - weight_force + Drag;
